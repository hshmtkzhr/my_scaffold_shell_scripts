...
for elm in j:
  ...
  ip     = elm.get("networkInterfaces", [{}])[0].get("networkIP", "")
  #
  created_at   = elm.get("creationTimestamp", "")
  machine_type = os.path.basename(elm.get("machineType", "")).rstrip()
  #
  zone = os.path.basename(elm.get("zone", "")).rstrip()
  tags = ",".join(elm.get("tags", {}).get("items", ""))
  #
  hostname = ""
  ...
  print("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}".format(
    name, hostname, machine_type, status, zone, ip, tags, created_at
    )
  )
' > $tmp.list
